...
- You chose a correct value of `stat(x, y)` when `x, y` is *outside* the
  ...
  - I chose stat(x, y) == 0 because the point lies outside the
    quarter-circle, so the indicator is 0 and 4 × 0 = 0.
- You chose a correct value of `stat(x, y)` when `x, y` is *inside* the
  circle. Why did you choose this value?
  - I chose stat(x, y) == 4 because the point lies inside the
    quarter-circle, so the indicator is 1 and 4 × 1 = 4.
...
``` r
z <- qnorm(1 - 0.05 / 2)  # z-value for 95% confidence ≈ 1.96


df_q1 %>%
  mutate(stat_val = stat(x, y)) %>%
  summarise(
    pi_est = mean(stat_val),
    se = sd(stat_val) / sqrt(n()),  # Standard error
    pi_lo = pi_est - z * se,
    pi_up = pi_est + z * se
  )
```
    ...
    ## 1  3.168 0.0513655 3.067325 3.268675
...
- Comment on the width of your CI(s). Would your estimate of $\pi$ be
  ...
  orbit)?
  - The estimate is probably good enough for roughly estimating the area
    because the width isn’t too large
  - The estimate is definitely not good enough for precise calculations
    because while the width isn’t too large, it is still big enough to
    cause problems in precise calculations
