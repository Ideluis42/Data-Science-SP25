...
-   You chose a correct value of `stat(x, y)` when `x, y` is *outside* the circle. Why did you choose this value?
    -   I chose stat(x, y) == 0 because the point lies outside the quarter-circle, so the indicator is 0 and 4 × 0 = 0.
-   You chose a correct value of `stat(x, y)` when `x, y` is *inside* the circle. Why did you choose this value?
    -   I chose stat(x, y) == 4 because the point lies inside the quarter-circle, so the indicator is 1 and 4 × 1 = 4.
...
-   Comment on the width of your CI(s). Would your estimate of $\pi$ be good enough for roughly estimating an area (e.g., to buy enough paint for an art project)? Would your estimate of $\pi$ be good enough for precisely calculating a trajectory (e.g., sending a rocket into orbit)?
    -   The estimate is probably good enough for roughly estimating the area because the width isn't too large
    -   The estimate is definitely not good enough for precise calculations because while the width isn't too large, it is still big enough to cause problems in precise calculations
